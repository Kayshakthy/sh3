...
namespace ArrisTest
{
    public partial class Login_Screen_Router_Status_Upstream_Validation
    {
        /// <summary>
        ...
        public void Validate_upstremChannel()
        {
        	
        	//Validate Upstream  bonded channels
        	Report.Info("Validate Upstream  bonded channels");
			Delay.Seconds(30);
			WebDocument webDocument = "/dom[@domain='192.168.0.1']";
			webDocument.WaitForDocumentLoaded();
			
			//validate the UpstremChannel(Upstream bonded channels) table rows & columns
...
//			string totalRowsCnt = "Total Rows(DownloadChannel1) : "+rowsCount.ToString();
			...
			Report.Info("Total Rows(UpstremChannel) ",totalRowsCnt);
...
//						Report.Info(col.GetInnerHtml().ToString());
				...
				Report.Failure("There are no Upstream bonded channels");
			}
			
			//validate the UpstremChannel1(Upstream bonded channels) table rows & columns
			TableTag myTable1 = webDocument.FindSingle("/dom[@domain='192.168.0.1']//table[#'UpstremChannel1']");
...
//			string totalRowsCnt = "Total Rows(DownloadChannel1) : "+rowsCount.ToString();
			...
			Report.Info("Total Rows(UpstremChannel1) ",totalRowsCnt1);
