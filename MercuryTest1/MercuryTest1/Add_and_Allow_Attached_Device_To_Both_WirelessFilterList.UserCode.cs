...
namespace ArrisTest
{
	public partial class Add_and_Allow_Attached_Device_To_Both_WirelessFilterList
	{
		/// <summary>
		...
		public void Choose_attached_device_to_add_2and5Ghz(string argument1, string argument2, string argument3, string argument4, string argument5, string argument6, string argument7)
		{
            ...
            //Count the total no of IPv4 port Filter rules (rows)
	     	...
	     	if(rulesCount == 0){
	     		...
	     		TestSuite.Current.GetTestCase("Add_Device_To_Both_Wireless_MAC_Address_List").Checked = false;
	     		return;
	     	}else{
...
//			        Report.Info(row.GetInnerHtml().ToString());
			        ...
			        int totalCol = cols.Count;
			        Report.Info(" Total Columns in the Row : ",totalCol.ToString());
			        int colNum = 1;
...
//			        	Report.Info("MacCol Info length : " + MacCol.InnerText.Length );
//			        	Report.Info(MacCol.GetInnerHtml().ToString());
			        		...
			        		//Report.Info("span count :  "+spanVals.Count);
			        		 foreach(SpanTag spanVal in spanVals){
			        			...
			        			if(macAddress == macAddress1){
									...
									*/
				        			string rst = "The following MAC_Filtering rule is selected to add to the filter list !!!";
									Report.Success(rst);
						        	//Report.Screenshot("MACFilter_Filtering",MacCol.Element,true);
						        	Report.Screenshot("MAC Filter_List",row.Element,true);
						        	Report.Screenshot();	//taking full screenshot
					        		break;
			        			}
			      ...
			      if(isMatch){
			        	...
			        	if(Wifi_Freq.Trim()==" WI-FI 2.4G"){
			        		Report.Info("This device is currently connected to  WI-FI 2.4G");
				...
				if(!isMatch){
					...
					TestSuite.Current.GetTestCase("Add_Device_To_Both_Wireless_MAC_Address_List").Checked = false;	//Add_Device_To_Both_Wireless_MAC_Address_List
				
				}else{
					TestSuite.Current.GetTestCase("Add_Device_To_Both_Wireless_MAC_Address_List").Checked = true;	//Add_Device_To_Both_Wireless_MAC_Address_List
