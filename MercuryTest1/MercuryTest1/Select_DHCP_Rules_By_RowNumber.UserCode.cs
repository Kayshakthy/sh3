...
namespace ArrisTest
{
    public partial class Select_DHCP_Rules_By_RowNumber
    {
        /// <summary>
        ...
        private void Init()
        {
            ...
            Report.Info("TotalRows(while loading the page):",TotalRows.ToString());
            
        }

        public void Select_DHCP_Rule_By_RowNumber(string argument1)
        {
...
//	     	Report.Info("Active DHCP Reserv list ",totalDHCPReservListStr);    
	     	if(rulesCount == 0){
			    ...
			    string ipAddress1="",macAddress1="";

			    Report.Info("Row Number to Delete : " + argument1);
...
//			        		Report.Info("----------------- click delete checkbox ------");
			        		String row_num = argument1.Trim();
			        		numVal = int.Parse(row_num);
			        		if(rowNum == numVal){
			        			isMatch = true;
...
//			        			Report.Info("delCheck ", delCheck.ToString());
			    ...
			    if((rowNum-1) == rulesCount  && !isMatch){	//if rule does not exist
					Report.Info(" ===== This Row number does not exist =====");
					Delay.Seconds(1);
